...
<template>
  <LayoutLight>
    ...
    <template v-slot:header-content>
      <HeaderContent :nav-link="{ label: 'Govern', path: `/g/${gatheringSlug}/govern` }" />
    </template>
    <template v-slot:command-band>
      <div class="max-w-screen-md mx-auto mb-4 text-center flex-center">
        <SearchInput
          ...
          :searching="false"
          :existing-tags="[]"
          @search="(search) => { filters.search = search }"
          @selected-tag="(tag) => { alert(tag) }"
        />
        <button
        ...
        >
          <div style="width: 1rem;" class="inline-block">
            <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="plus" role="img" xmlns="http://www.w3.org/2000/svg" style="max-height: 100%; max-width: 100%;" viewBox="0 0 512 512" class="svg-inline--fa fa-plus fa-w-14"><path fill="currentColor" d="M416 208H272V64c0-17.67-14.33-32-32-32h-32c-17.67 0-32 14.33-32 32v144H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h144v144c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32V304h144c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z" class=""></path></svg>
          </div>
      ...
      <div class="max-w-screen-md mx-auto text-center flex-center flex-wrap">
        <TagButton
          v-for="tag in (docs?.remindingTags?.length ? docs?.remindingTags : tags ?? []).slice(0, 10).map(tag => ({ id: tag.id, text: tag.title, title: tag.title }))"
          :key="`tag-${tag.title}`"
          :tag="tag"
    ...
    <template v-slot:content>
      ...
      >
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
          <DialogPanel class="w-full max-w-[40rem] rounded bg-white p-8">
            <AddNewModal
              :existing-tags="tags.map(tag => ({ text: tag.title, value: tag.title }))"
      ...
      <div v-if="newDocumentModal || tipModal" class="backdrop" @click="() => { newDocumentModal = false; tipModal = false }"></div>
    </template>
